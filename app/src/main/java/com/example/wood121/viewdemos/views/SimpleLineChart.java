...
public class SimpleLineChart extends View {
    ...
    protected void onDraw(Canvas canvas) {
        ...
        axisPaint.setColor(Color.parseColor("#dd7e6b"));
...
//            Log.e("wing", xPoints[i] + "");
            ...
            canvas.drawLine(5, 0, 200, 0, backPaint);
            ...
            for (int i = 0; i < mXAxis.length; i++) {
                ...
                //画点: x时间轴是固定的，y是输入数值计算划分的格子，点是需要表示出来点数据
                canvas.drawCircle(xPoints[i], yPoints[mPointMap.get(i)], mPointRadius + 5, pointPaint);
                if (i > 0) {
                    canvas.drawLine(xPoints[i - 1], yPoints[mPointMap.get(i - 1)], xPoints[i], yPoints[mPointMap.get(i)], linePaint);
                    canvas.drawCircle(xPoints[i - 1], yPoints[mPointMap.get(i - 1)], mPointRadius, innerPointPaint);
                    canvas.drawCircle(xPoints[i], yPoints[mPointMap.get(i)], mPointRadius, innerPointPaint);
                }
            }
